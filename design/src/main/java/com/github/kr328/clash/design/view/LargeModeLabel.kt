...
import com.github.kr328.clash.design.databinding.ComponentLargeModeLabelBinding
...
class LargeModeLabel @JvmOverloads constructor(
    context: Context,
    attributeSet: AttributeSet? = null,
    @AttrRes defStyleAttr: Int = 0,
    @StyleRes defStyleRes: Int = 0
) : FrameLayout(context, attributeSet, defStyleAttr, defStyleRes) {
    private val binding = ComponentLargeModeLabelBinding
    ...
    init {
        ...
        context.theme.obtainStyledAttributes(
            attributeSet,
            R.styleable.LargeModeLabel,
            defStyleAttr,
            defStyleRes
        ).apply {
            try {
                icon = getDrawable(R.styleable.LargeModeLabel_icon)
                text = getString(R.styleable.LargeModeLabel_text)
                subtext = getString(R.styleable.LargeModeLabel_subtext)
            } finally {
                recycle()
            }
        }
